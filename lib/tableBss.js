...
exports.getTablesBss = function getTablesBss(req, res) {
    var query = 'SELECT idtabla_bss, nombre as name, descripcion as description, DATE_FORMAT(fecha_creacion, \"%d/%m/%Y %H:%i:%s\") AS FechaCreacion,' +
...
/** */
exports.idTableBss = async function(listOfValues) {
    if (Array.isArray(listOfValues)==false){
        // marcar error
        logging.Error("idTableBss. La lista pasada no es un array");
        return -1;
    }
    if (listOfValues.length != 6){
        // Marcar error
        logging.Error("idTableBss. La lista pasada no tiene el tamaño de una tabla bss (6)");
        return -1;
    }
    var query = "SELECT idtabla_bss FROM tablas_bss WHERE" +
        " valor0=" + listOfValues[0] +
        " AND valor1=" + listOfValues[1] +
        " AND valor2=" + listOfValues[2] +
        " AND valor3=" + listOfValues[3] + 
        " AND valor4=" + listOfValues[4] + 
        " AND valor5=" + listOfValues[5] + ";"
    var res = await ug5kdb.QuerySync(query);
    if (res.error){
        // Marcar error.
        logging.Error("idTableBss. Error en la consulta: ", query, res.error);
        return -1;
    }
    var id = res.data.length==0 ? -1 : res.data[0].idtabla_bss;
    logging.Error("idTableBss. Table ", listOfValues, id);
    return id;
}
