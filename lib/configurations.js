...
var myLibTbBss = require('./tableBss.js');
...
exports.getConfigurations = function getConfigurations(req, res) {
    ...
    connection.connect(function (err, usrs) {
        if (err) {
            ...
            var query = connection.query("SELECT c.idconfiguracion as idCFG, c.nombre as name, c.descripcion as description," +
                "c.region, DATE_FORMAT(c.fecha_activacion, \"%d/%m/%Y %H:%i:%s\") AS fecha_activacion, c.activa as activa " +
                "FROM configuraciones c", function (err, rows, fields) {
    ...
    connection.connect(function (err) {
        if (err) {
            ...
            var strQuery = 'SELECT c.idconfiguracion AS idCFG, c.nombre AS NAME, c.descripcion AS description, c.activa AS activa, ' +
                ...
                'e.idemplazamiento AS idEMPLAZAMIENTO, DATE_FORMAT(c.fecha_activacion, "%d/%m/%Y %H:%i:%s") AS fecha_activacion ' +
...
exports.postConfiguration = function postConfiguration(req, res, newConfiguration, f) {
    ...
    connection.connect(function (err) {
        if (err) {
            ...
            var query = connection.query('INSERT INTO configuraciones (nombre, descripcion, activa) ' +
                'VALUES (?,?,?)', [newConfiguration.name, newConfiguration.description, newConfiguration.activa],
                function (err, result) {
                    logging.Trace(query.sql);
                    connection.end();
                    if (err) {
...
exports.putConfiguration = async function putConfiguration(oldIdConf, cfg, f) {
    ...
    var res = await ug5kdb.QuerySync('UPDATE configuraciones SET nombre=?, descripcion=?, activa=?, fecha_activacion=UTC_TIMESTAMP() WHERE idconfiguracion=?',
        [cfg.name, cfg.description, cfg.activa, cfg.idCFG]);
    if (!res.error) {
        ...
        if (err) {
            ...
            var query = connection.query('DELETE FROM configuraciones WHERE idconfiguracion=?', [cfg],
                function (err, result) {
                    ...
                    connection.end();
                    if (err) {
                        logging.Error("Error executing query: " + err.message);
                        f({ data: err.message });
                    }
                    else
                        f({ data: 'OK' });
...
exports.copyConfiguration = function copyConfiguration(idSourceConfig, trgt, f) {
    ...
    connection.connect(function (err) {
        if (err) {
            logging.Error("Error connection to 'U5K-G' database.");
            return f({data: "Error al conectarse a base de Datos"});
        }
        else {
            var newIdCfg = -1;
            var oldSiteData = [];
            ...
            var insertNewSites = async function /*insertNewSites*/(callback) {
                ...
                for (var i = 0; i < oldSiteData.length; i++) {
                    ...
                    if (!res.error) {
                        ...
                        return callback('insertNewSites error');
                    }
                }
                return callback(null);
            };
                ...
                for (var i = 0; i < oldGtwData.length; i++) {
                    if (oldGtwData[i].idpasarela != null) {
                        ...
                        query = 'INSERT INTO pasarelas (emplazamiento_id,nombre,ip_virtual,ultima_actualizacion,ip_cpu0,' +
                            ...
                            ',' + sppe +
                            ',' + dvrrp +
                            ',' + oldGtwData[i].supervisionTlf +
                            ',' + oldGtwData[i].refresher +
                            ');';
                        res = await ug5kdb.QueryMultiInsertSync(query);
                        if (!res.error) {
                            var element = new Object();
                        ...
                        logging.Error('InsertNewGtws error: No idPasarela');
                    }
                }
                return callback(null);
            };
            ...
            var insertNewGtwIps = async function /*insertNewGtwIps*/(callback) {
                ...
                for (var i = 0; i < oldGtwIpData.length; i++) {
                    if (oldGtwIpData[i].pasarela_id != null) {
                        ...
                        logging.Error('insertNewGtwIps error: No idPasarela');
                    }
                }
                return callback(null);
            };
            ...
            var insertNewGtwRadioRes = async function /*insertNewGtwRadioRes*/(callback) {
                ...
                for (var i = 0; i < oldGtwRadioResData.length; i++) {
                    if (oldGtwRadioResData[i].pasarela_id != null) {
                        ...
                        logging.Error('insertNewGtwRadioRes error: No idPasarela');
                    }
                }
                return callback(null);
            };
                ...
                for (var i = 0; i < oldGtwTfnoResData.length; i++) {
                    if (oldGtwTfnoResData[i].pasarela_id != null) {
                        ...
                        query = 'INSERT INTO recursos_telefono (pasarela_id,fila,columna,nombre,' +
                            ...
                            'llamada_automatica, RespuestaSIP_ATSR2, TmTonoBloqueo, TmBloqueoLib, iControlTmLlam, iTmMaxConversacion' +
                            ') VALUES ' +
                        ...
                        logging.Error('insertNewGtwTfnoRes error: No IdPasarela');
                    }
                }
                return callback(null);
            };
            ...
            var insertNewRadioResLista_Uris = async function /*insertNewRadioResLista_Uris*/(callback) {
                ...
                for (var i = 0; i < oldGtwRadioResLista_UrisData.length; i++) {
                    if (oldGtwRadioResLista_UrisData[i].recurso_radio_id != null) {
                        ...
                        logging.Error('insertNewRadioResLista_Uris error. No resource ID')
                    }
                }
                return callback(null);
            };
            ...
            var insertNewTfnoResRangos_Ats = async function /*insertNewTfnoResRangos_Ats*/(callback) {
                ...
                for (var i = 0; i < oldGtwTfnoResRangos_AtsData.length; i++) {
                    if (oldGtwTfnoResRangos_AtsData[i].recurso_telefonico_id != null) {
                        ...
                        logging.Error('insertNewTfnoResRangos_Ats error: No resource id');
                    }
                }
                return callback(null);
            };
            ...
            async.waterfall([
                ...
                if (err) {
                    ...
                    // Borrar lo creado.
                    if (newIdCfg !== -1) {
                        connection.query('DELETE FROM configuraciones WHERE idconfiguracion=?', [newIdCfg],
                        function (err, result) {
                            logging.Info("Borrando Cfg Creada con Errores", newIdCfg, err ? err : 'OK');
                        });
                    }
...
exports.getConfigurationsByGtW = function (IdGTW, res) {
    var cfgs = [];
    var mysql = require('mysql');
    var connection = ug5kdb.GetDbConnection();/* mysql.createConnection({
	  host     : gcfg.Ulises.dbhost,
    ...
    connection.connect(function (err, usrs) {
        if (err) {
            ...
            var query = connection.query("SELECT e.configuracion_id FROM emplazamientos e " +
                "INNER JOIN configuraciones as c on e.configuracion_id = c.idconfiguracion WHERE idemplazamiento = ? "
                , [IdGTW], function (err, rows, fields) {
                    logging.Trace(query.sql);
                    connection.end();
                    if (err) {
                        if (err.fatal) {
...
exports.checkIps = function (ip, idCFG, res) {
    var cfgs = [];
    var mysql = require('mysql');
    var connection = ug5kdb.GetDbConnection();/* mysql.createConnection({
      host     : gcfg.Ulises.dbhost,
    ...
    connection.connect(function (err) {
        if (err) {
            ...
            queryCgwCfg = connection.query('Select c.nombre as nombre_conf, p.nombre, p.ip_virtual, p.ip_cpu0,p.ip_cpu1, c.activa ' +
                'from configuraciones as c ' +
                'join emplazamientos as e on c.idconfiguracion=e.configuracion_id ' +
                'join pasarelas as p on e.idemplazamiento=p.emplazamiento_id ' +
                'where activa = 1 and (p.ip_virtual = ? or p.ip_cpu0 = ? or p.ip_cpu1 = ?) ' +
...
exports.resetGatewaysSynchroState = function resetGatewaysSynchroState(f) {
    ...
    connection.connect(function (err) {
        if (err) {
            logging.Error(`Error connection to 'U5K-G' database => ${err}.`);
            return f(err);
        }
        else {
            ...
            var insertGtwData = function /*insertGtwData*/(callback) {
                ...
                var query = connection.query('INSERT INTO pasarelas (emplazamiento_id,nombre,ip_virtual,ultima_actualizacion,' +
                    ...
                    newCgwData.servicios.grab.rtsp_port, newCgwData.servicios.grab.rtsp_ip, newCgwData.servicios.grab.rtspb_ip,
                        sppe, dvrrp,
                    /** 202210 Nuevos parametros de configuración */
                    newCgwData.servicios.sip.SupervisionTlf, newCgwData.servicios.sip.Refresher],
                    function (err, result) {
                        logging.Trace(query.sql);
                        if (err) {
                            return f(err ? err : 'NO_DATA');
                ...
                if (flagResources.resRadio) {
                    for (var i = 0; i < newCgwData.recursos.length; i++) {
                        if (newCgwData.recursos[i].Radio_o_Telefonia == 1) {
                            ...
                            // AGL. 20210615. Si FID (colateral.name) viene en blanco poner un 0.0 en BDT
                            var fid = newCgwData.recursos[i].radio.colateral.name == "" ? "0.000" : newCgwData.recursos[i].radio.colateral.name;
                            // AGL. RM4890. En los ficheros importados desde pasarela no se identifica el id de tabla bss
                            // var idBss = newCgwData.recursos[i].idBss ? newCgwData.recursos[i].idBss : -1;
                            // AGL. RM4891. Obtener el id de la base de datos.
                            var idBss = await myLibTbBss.idTableBss(newCgwData.recursos[i].radio.tabla_indices_calidad);
                            ...
                            queryString = 'INSERT INTO recursos_radio (pasarela_id,fila,columna,nombre,codec,' +
                                ...
                                '' + newCgwData.recursos[i].restriccion + ',' + idBss + ')';
                    ...
                    for (var i = 0; i < newCgwData.recursos.length; i++) {
                        if (newCgwData.recursos[i].Radio_o_Telefonia == 2) {
                            ...
                            queryString = 'INSERT INTO recursos_telefono (pasarela_id,fila,columna,nombre,codec,' +
                                ...
                                ',' + additional_superv_options +
                                ',' + additional_itiporespuesta +
                                ',' + newCgwData.recursos[i].LlamadaAutomatica +
                                ',' + newCgwData.recursos[i].telefonia.RespuestaSIP_ATSR2 +
                                ',' + newCgwData.recursos[i].telefonia.TmTonoBloqueo +
                                ',' + newCgwData.recursos[i].telefonia.TmBloqueoLib +
                                ',' + newCgwData.recursos[i].telefonia.iControlTmLlam +
                                ',' + newCgwData.recursos[i].telefonia.iTmMaxConversacion +
                                ')';
