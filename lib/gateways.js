...
var config = require('../configUlises.json');
    // 20221115. AGL. Incluye en el template los datos de RespuestasR2-SIP de SDC91.
var sipr2 = require("./sipr2mng.js");
...
exports.setGatewayPendingToUpdateToZero = function setGatewayPendingToUpdateToZero(IdGateway, bdtDate, f) {
    var mysql = require('mysql');
    ...
    connection.connect(function (err, usrs) {
        if (err) {
            ...
            var queryUpdate = connection.query(
                'UPDATE pasarelas SET pendiente_actualizar=0, ultima_actualizacion=? WHERE idpasarela=?',
                [/*getCurrentDateLocalISO()*/bdtDate, IdGateway],
                function (err, result) {
                    logging.Trace(queryUpdate.sql);
                    connection.end();
                    if (err)
        ...
        if (err) {
                        ...
                        function (err, rows) {
                            try {
                                if (err || rows.length == 0) {
                                    logging.Error(err);
                                    return callback({error: rows.length == 0 ? 'NO_DATA' : err});
                                }
                                else {
            ...
            var selectDataPasarelas = function /*selectDataPasarelas*/(idGtw, callback) {
                var queryPasarela = connection.query('SELECT p.nombre as name, c.nombre as idConf, ' +
                    ...
                    /** 202210 Nuevos parametros de configuración */
                    'p.supervisionTlf, p.refresher ' +
                    'FROM pasarelas p ' +
                    'LEFT JOIN emplazamientos e ON p.emplazamiento_id=e.idemplazamiento ' +
                    'LEFT JOIN configuraciones c ON e.configuracion_id=c.idconfiguracion ' +
                    ...
                    function (err, rows) {
                        try{
                            if (err) {
                                ...
                                template.servicios.sip.PuertoLocalSIP = rows[0].puerto_sip;
                                /** 202210 Nuevos parametros de configuración */
                                template.servicios.sip.SupervisionTlf = rows[0].supervisionTlf;
                                template.servicios.sip.Refresher = rows[0].refresher;
    
                                // REGISTRARS Y PROXIES    
            ...
            var selectDataTfno = function /*selectDataTfno*/(idGtw, template, callback) {
                var query = connection.query('SELECT idrecurso_telefono,nombre,columna,fila,codec,clave_registro,' +
                    ...
                    'llamada_automatica, iControlTmLlam, iTmMaxConversacion, RespuestaSIP_ATSR2, TmTonoBloqueo, TmBloqueoLib '+
                        ...
                        try{
                            if (err) {
                                ...
                                for (var i = 0; i < rows.length; i++) {
                                    ...
                                    /** 20221019. Nuevo parametro para linea telefonica */
                                    recurso.LlamadaAutomatica = rows[i].llamada_automatica;
                                    recurso.telefonia.iControlTmLlam = rows[i].iControlTmLlam;
                                    recurso.telefonia.iTmMaxConversacion = rows[i].iTmMaxConversacion;
                                    recurso.telefonia.RespuestaSIP_ATSR2 = rows[i].RespuestaSIP_ATSR2;
                                    recurso.telefonia.TmTonoBloqueo = rows[i].TmTonoBloqueo;
                                    recurso.telefonia.TmBloqueoLib = rows[i].TmBloqueoLib;
                        ...
                        try{
                            if (err) {
                                ...
                                for (var i = 0; i < rows.length; i++) {
                                    ...
                                    logging.Trace("(gateways).getAll (selectDataRadio) ", configName, ipGtw, recurso.IdRecurso);
                                    ...
                                    recurso.radio.colateral.name = rows[i].frecuencia == 0.0 ? "" : rows[i].frecuencia.toFixed(3);
                                ...
                                for (var j = 0; j < template.recursos.length; j++) {
                                    ...
                                    for (var i = 0; i < rows.length; i++) {
                                        ...
                                        if (rows[i].idrecurso_radio == template.recursos[j].idDBRadio) {
                                            ...
                                            var item = rows[i];
                                            if (item.tipo==null){
                                                logging.Trace('Recurso Radio sin URIS asociadas. Recurso ', item.idrecurso_radio);
                                            } else {
                                    ...
                                    for (var i = 0; i < rows.length; i++) {
                                        if (rows[i].idrecurso_telefono == template.recursos[j].idDBTfno) {
                                            if (rows[i].tipo == 0 && rangosOrgCounter < 4) {
                                                ...
                                                rangosOrgCounter++;
                                            } else if (rows[i].tipo == 1 && rangosDstCounter < 4) {
                                                ...
                                                logging.Error("Error Copiando rangos ATs en pasarela ", idGtw, rangosOrgCounter, rangosDstCounter);
                                            }
                                        }
                                    }
                                }
            ...
            var selectTablaBss = async function /*selectTablaBss*/(idGtw, template, callback) {
                ...
                try {
                    for (var i = 0; i < template.recursos.length; i++) {
                        template.recursos[i].idBss = template.recursos[i].idBss ? template.recursos[i].idBss : 0;
                        if (template.recursos[i].Radio_o_Telefonia == 1 && (template.recursos[i].radio.tipo == 4 ||
                            ...
                            console.log("Leyendo Tabla Bss", template.recursos[i].idBss);
                        ...
                        try {
                            if (err) {
                                ...
                                for (var i = 0; i < rows.length; i++) {
                                    ...
                                    user.perfil |= (rows[i].perfil & 0x0100) != 0 ? 0x0040 : 0x0000;	// Perfil Administrador GWS.    
                                    user.perfil |= (rows[i].perfil & 0x0040) != 0 ? 0x0040 : 0x0000;	// Perfil Administrador.    
                                    if (user.perfil != 0) {
            ...
            // 20221115. AGL. Incluye en el template los datos de RespuestasR2-SIP de SDC91.
            var selectAtsSipResponses = function(template, callback)  {
                sipr2.Get((response)=>{
                    // console.log("sipr2 response => ", response.error, response.data);
                    if (!response.error){
                        logging.Trace("selectAtsSipResponses ok");
                        template.modoSDC91.respuestasSIPtoATSR2 = response.data;
                        // console.log("modoSCD91", template.modoSDC91, callback);
                        // response.data.forEach(reg=>{
                        //     template.modoSDC91.respuestasSIPtoATSR2.push(reg);
                        // });
                        return callback(null, template);
                    }
                    else {
                        logging.Error("selectAtsSipResponses Error => ", response.data);
                        return callback("Error: " + response.data, template);
                    }
                });
            };
            async.waterfall([
                    ...
                    selectListaUsers
                    // 20221115. AGL. Incluye en el template los datos de RespuestasR2-SIP de SDC91.
                    ,selectAtsSipResponses
                ], function (err, result) {
                    connection.end();
                    if (err){
                        logging.Error(err);
                        f({data: 'Error procesando la peticion'});
                    } else {
                        f(result, idGtw);
                    }
                }
    ...
    connection.connect(function (err, usrs) {
        if (err) {
            ...
            var query = connection.query('SELECT p.idpasarela as idCGW,e.nombre as nameSite, ' +
                'p.nombre as name, 1 as Viva,1 as Activa, DATE_FORMAT(p.ultima_actualizacion, "%d/%m/%Y %H:%i:%s") AS ultima_actualizacion, p.pendiente_actualizar, ' +
                'CONCAT(p.ip_virtual, " ( ", p.ip_cpu0, " , ", p.ip_cpu1, " )") as ipInfo ' +
                'FROM pasarelas p ' +
                'LEFT JOIN emplazamientos e ON e.idemplazamiento=p.emplazamiento_id ' +
                'LEFT JOIN configuraciones c ON c.idconfiguracion=e.configuracion_id ' +
    ...
    connection.connect(function (err) {
        if (err) {
            ...
            if (cfg == null) { //Venimos de ver la gtw al pulsar sobre ella
                query = connection.query('SELECT p.idpasarela as idCGW, c.nombre as conf_name, p.emplazamiento_id as EMPLAZAMIENTO_idEMPLAZAMIENTO,  ' +
                    'p.nombre as name, e.nombre as emplazamiento, p.ip_virtual as ipv, ' +
                    'DATE_FORMAT(p.ultima_actualizacion, \"%d/%m/%Y %H:%i:%s\") AS ultima_actualizacion, p.ip_cpu0, p.ip_gtw0, p.mask_cpu0, p.ip_cpu1, p.ip_gtw1, p.mask_cpu1, ' +
                    ...
                    'p.sppe, p.dvrrp, p.pendiente_actualizar, ' +
                    /** 202210 Nuevos parametros de configuración */
                    'p.refresher, p.supervisionTlf ' +
                    'FROM pasarelas p ' +
                    'LEFT JOIN emplazamientos e ON p.emplazamiento_id = e.idemplazamiento ' +
                    'LEFT JOIN configuraciones c ON c.idconfiguracion = e.configuracion_id ' +
...
exports.delGateway = function delGateway(req, res, gtw) {
    ...
    connection.connect(function (err) {
        if (err) {
            ...
            var getIdCfg = connection.query('SELECT c.idconfiguracion ' +
                'FROM pasarelas p ' +
                'LEFT JOIN emplazamientos e ON p.emplazamiento_id=e.idemplazamiento ' +
                'LEFT JOIN configuraciones c ON e.configuracion_id=c.idconfiguracion ' +
                'WHERE p.idpasarela=?', [gtw], function (err, result) {
                    logging.Trace(getIdCfg.sql);
                    if (err) {
                        connection.end();
                        return f({ error: err.message });
                    } else {
                        var idConfig = result && result.length > 0 && result[0].idconfiguracion;
                        var querySel = connection.query('DELETE FROM pasarelas WHERE idpasarela=? ', gtw,
                            function (err, result) {
                                ...
                                if (err) {
                                    ...
                                    var resultUpdate = connection.query('UPDATE configuraciones SET fecha_activacion=UTC_TIMESTAMP() WHERE idconfiguracion=?',
                                        [idConfig], function (err, resultUpdate) {
    ...
    connection.connect(function (err) {
        if (err) {
            ...
            var querySel = connection.query('SELECT rt.idrecurso_telefono,rt.nombre,rt.codec,rt.clave_registro,rt.precision_audio,' +
                ...
                /** 202210. Nuevos tipos de recurso*/
                'rt.llamada_automatica, rt.iControlTmLlam, rt.iTmMaxConversacion, rt.RespuestaSIP_ATSR2, rt.TmTonoBloqueo, rt.TmBloqueoLib ' +
                'FROM recursos_telefono rt ' +
...
exports.insertRadioRes4Gateway = function insertRadioRes4Gateway(rr, f) {
    ...
    connection.connect(function (err) {
        if (err) {
            ...
            var selQuery = connection.query('SELECT c.activa, c.idconfiguracion ' +
                'FROM pasarelas p ' +
                'LEFT JOIN emplazamientos e ON p.emplazamiento_id=e.idemplazamiento ' +
                'LEFT JOIN configuraciones c ON e.configuracion_id=c.idconfiguracion ' +
                ...
                function (err, rows) {
                    if (err) {
                        ...
                        var querySel = connection.query('INSERT INTO recursos_telefono (pasarela_id,fila,columna,nombre,' +
                            'codec,clave_registro,ajuste_ad,ajuste_da,precision_audio,tipo_interfaz_tel,uri_telefonica,deteccion_vox,umbral_vox,' +
                            ...
                            ')',
                            [
                                tr.pasarela_id, tr.fila, tr.columna, tr.nombre, tr.codec, tr.clave_registro, tr.ajuste_ad, tr.ajuste_da, tr.precision_audio,
                                ...
                                iDetLineaAB, iEnableNoED137, itiporespuesta, additional_uri_remota, additional_superv_options, additional_itiporespuesta,
                                /** 202210. Nuevos recursos */
                                tr.llamada_automatica, tr.iControlTmLlam, tr.iTmMaxConversacion, tr.RespuestaSIP_ATSR2, tr.TmTonoBloqueo, tr.TmBloqueoLib
                            ],
                            function (err, result) {
                                logging.Trace(querySel.sql);
...
exports.updateTfnoRes4Gateway = function updateTfnoRes4Gateway(tr, resId, f) {
    ...
    logging.Info("(gateways).updateTfnoRes4Gateway ", tr, resId);
    var activa = 0;
    connection.connect(function (err) {
        if (err) {
            logging.Error("Error connention to 'U5K-G' database.");
            ...
            var selQuery = connection.query('SELECT c.activa, c.idconfiguracion ' +
                ...
                function (err, rows) {
                    if (err) {
                        ...
                        var query = connection.query('UPDATE recursos_telefono SET nombre=?,clave_registro=?,ajuste_ad=?,ajuste_da=?,precision_audio=?,' +
...
//** 20230208. AGL. Rutina Sincrona para updateTfnoRes4GatewaySync */
...
exports.copyGateway = function (sourceIdGateway, targetNameGateway,
    ip0TargetGateway, ip1TargetGateway, ipvTargetGateway, ipGateway, f) {
    var mysql = require('mysql');
    var connection = ug5kdb.GetDbConnection();/* mysql.createConnection({
        host: gcfg.Ulises.dbhost,
        user: gcfg.Ulises.dbuser,
    ...
    logging.Info("(gateways).copyGateway ", sourceIdGateway, targetNameGateway, ip0TargetGateway, ip1TargetGateway, ipvTargetGateway, ipGateway);
        ...
        if (err) {
            ...
            var insertNewGtw = async function /*insertNewGtw*/(callback) {
                ...
                var stringQuery = 'INSERT INTO pasarelas (emplazamiento_id,nombre,ip_virtual,ultima_actualizacion,ip_cpu0,' +
                    ...
                    /** 202210 Nuevos parametros de configuración */
                    'supervisionTlf, refresher) ' +
                    'VALUES (' + oldGtwData[0].emplazamiento_id + ',"' + targetNameGateway + '","' + ipvTargetGateway + '",NOW()' +
                    ',"' + ip0TargetGateway + '","' + ipGateway + '","' + oldGtwData[0].mask_cpu0 + '"' +
                    ',"' + ip1TargetGateway + '","' + ipGateway + '","' + oldGtwData[0].mask_cpu1 + '"' +
                    ...
                    ',' + dvrrp +
                    /** 202210 Nuevos parametros de configuración */
                    ',' + oldGtwData[0].supervisionTlf +
                    ',' + oldGtwData[0].refresher +
                    ');';
                stringQuery = stringQuery.replace('\"null\"', 'null');
                var query = connection.query(stringQuery, function (err, result) {
                    logging.Trace(query.sql);
                    if (err) {
                        logging.Error(err);
                        return callback(result && result.length && result.length == 0 ? 'NO_DATA' : err);
                    }
                    else {
                ...
                for (var i = 0; i < oldGtwTfnoResData.length; i++) {
                    if (oldGtwTfnoResData[i].pasarela_id != null) {
                        ...
                        query = 'INSERT INTO recursos_telefono (pasarela_id,fila,columna,nombre,' +
                            ...
                            /** 202210. Nuevos recursos */
                            ',' + oldGtwTfnoResData[i].llamada_automatica +
                            ',' + oldGtwTfnoResData[i].iControlTmLlam +
                            ',' + oldGtwTfnoResData[i].iTmMaxConversacion +
                            ',' + oldGtwTfnoResData[i].RespuestaSIP_ATSR2 +
                            ',' + oldGtwTfnoResData[i].TmTonoBloqueo +
                            ',' + oldGtwTfnoResData[i].TmBloqueoLib +
                            ');';
        ...
        if (err) {
            ...
            query = connection.query('INSERT INTO pasarelas (emplazamiento_id,nombre,ip_virtual,ip_cpu0,ip_gtw0,' +
                ...
                [idSite, gtw.nombre, gtw.ipv, gtw.ipb1, gtw.ipg1, gtw.msb1, gtw.ipb2, gtw.ipg2, gtw.msb2, gtw.puerto_snmp,
                    ...
                    gtw.tiempo_sesion, gtw.puerto_rtsp, gtw.servidor_rtsp, gtw.servidor_rtspb, sppe, dvrrp, gtw.supervisionTlf, gtw.refresher],
                function (err, rows) {
                    logging.Trace(query.sql);
                    if (err) {
                        connection.end();
                        ...
                        var newInsertCgwId = rows.insertId;
                        var getIdCfg = connection.query('SELECT c.idconfiguracion ' +
                            'FROM pasarelas p ' +
                            'LEFT JOIN emplazamientos e ON p.emplazamiento_id=e.idemplazamiento ' +
                            'LEFT JOIN configuraciones c ON e.configuracion_id=c.idconfiguracion ' +
                            'WHERE p.idpasarela=?', [newInsertCgwId], function (err, result) {
                                logging.Trace(getIdCfg.sql);
                                if (err) {
                                    connection.end();
                                    return f({ error: err.message });
                                    ...
                                    var idConfig = result && result.length > 0 && result[0].idconfiguracion;
                                    var resultUpdate = connection.query(
                                        'UPDATE configuraciones SET fecha_activacion=UTC_TIMESTAMP() WHERE idconfiguracion=?',
                                        [idConfig], function (err, resultUpdate) {
...
exports.updateGateWayonSite = async function updateGateWayonSite(gtw, idGtw, f) {
    ...
    var strQuery = 'SELECT configuraciones.idconfiguracion, configuraciones.activa FROM configuraciones ' +
    ...
    var pendiente_actualizar = gtw.pendiente_actualizar;
    var configId = res.data && res.data[0] ? res.data[0].idconfiguracion : null;
    var sppe = gtw.sppe ? gtw.sppe : 0;
    var dvrrp = gtw.dvrrp ? gtw.dvrrp : 2000;
    var supervisionTlf = gtw.supervisionTlf ? 1 : 0;

    strQuery = 'UPDATE pasarelas SET nombre=?,ip_virtual=?,ip_cpu0=?,ip_gtw0=?,' +
        'mask_cpu0=?,ip_cpu1=?,ip_gtw1=?,mask_cpu1=?,puerto_snmp=?,puerto_servicio_snmp=?,' +
        'periodo_supervision=?,snmpv2=?,comunidad_snmp=?,puerto_servicio_web=?,tiempo_sesion=?, ' +
        'puerto_rtsp=?,servidor_rtsp=?,servidor_rtspb=?,pendiente_actualizar=?, sppe=?, dvrrp=?, ' +
        /** 202210 Nuevos parametros de configuración */
        'refresher=?, supervisionTlf=? ' +
        'WHERE idpasarela=?';
    ...
    gtw.tiempo_sesion, gtw.puerto_rtsp, gtw.servidor_rtsp, gtw.servidor_rtspb, pendiente_actualizar,
        sppe, dvrrp, gtw.refresher, supervisionTlf,
        idGtw];
    ...
    if (res.error) {
        ...
        if (res.error) {
            return f({ error: 'Error en consulta: ' + strQuery + res.error });
        } else {
            var lista_ips = getFormattedIps(gtw);
            if (lista_ips.length > 0) {
                var queries = '';
            ...
            strQuery = 'UPDATE configuraciones SET fecha_activacion=UTC_TIMESTAMP() WHERE idconfiguracion=' + configId;
            res = await ug5kdb.QuerySync(strQuery);
            if (res.error) {
                return f({ error: 'Error en consulta: ' + strQuery + res.error });
            }
    ...
    connection.connect(function (err) {
        if (err) {
            ...
            var queryRadio = connection.query('SELECT rr.idrecurso_radio, rr.columna, rr.fila, ' +
                'rr.nombre, rr.frecuencia, rr.tipo_agente, rr.precision_audio, rr.metodo_bss FROM recursos_radio rr WHERE rr.pasarela_id=?', idGtw,
                function (err, rows) {
                    ...
                    if (err) {
                        ...
                        recursosRadio = rows;
                        var queryTfno = connection.query('SELECT rt.idrecurso_telefono, rt.columna, rt.fila, rt.nombre, rt.tipo_interfaz_tel, rt.precision_audio ' +
                        ...
                        myLibResources.getResource(h.idRECURSO, function (res) {
                            myLibResources.getTelephoneParameters(h.idRECURSO, function (param) {
                                if (param.parametros != null) {
                                    ...
                                    recursos[h.P_rank].telefonia.llamada_automatica = param.parametros.llamada_automatica;
                                    recursos[h.P_rank].telefonia.iControlTmLlam = param.parametros.iControlTmLlam;
                                    recursos[h.P_rank].telefonia.iTmMaxConversacion = param.parametros.iTmMaxConversacion;
                                }
                                callback();
                            });
                        });
...
exports.getGatewaysIpInConfig = function (ip, idCfg, idUpdatedCgw, f) {
    ...
    connection.connect(function (err, usrs) {
        if (err) {
            ...
            var query = connection.query('SELECT p.ip_virtual as ip ' +
                'FROM pasarelas p ' +
                'LEFT JOIN emplazamientos e ON p.emplazamiento_id=e.idemplazamiento ' +
                'LEFT JOIN configuraciones c ON e.configuracion_id=c.idconfiguracion ' +
