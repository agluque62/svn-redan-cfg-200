...
exports.setGatewayPendingToUpdateToZero = function setGatewayPendingToUpdateToZero(IdGateway, f) {
    var mySql = require('mySql');
    var connection = ug5kdb.GetDbConnection();

    console.log(getCurrentDateLocalISO());
    ...
    connection.connect(function (err, usrs) {
        if (err) {
            ...
            var queryUpdate = connection.query(
                ...
                [getCurrentDateLocalISO(), IdGateway],
                function (err, result) {
                    logging.Trace(queryUpdate.sql);
