                        ...
                        var queryInsert = connection.query('INSERT INTO emplazamientos (nombre, configuracion_id) ' +
                            ...
                            function (err, result) {
                                ...
                                if (err) {
                                    ...
                                    var res = connection.query('UPDATE configuraciones SET fecha_activacion=UTC_TIMESTAMP() WHERE idconfiguracion=?',
                                        [site.cfg_idCFG], function (err, res) {
...
exports.putSite = async function putSite(idSite, idCfg, nameSite, f) {
    ...
    if (!res.error) {
        ...
        if (!res.error) {
            ...
            if (!res.error) {
                res = await ug5kdb.QuerySync('UPDATE configuraciones SET fecha_activacion=UTC_TIMESTAMP() WHERE idconfiguracion=?', [idCfg]);
                if (!res.error) {
                    connection.end();
...
exports.deleteSite = function deleteSite(site, f) {
    ...
    connection.connect(function (err, usrs) {
        if (err) {
            ...
            var getIdCfg = connection.query('SELECT configuracion_id FROM emplazamientos WHERE idemplazamiento=?', [site], function (err, result) {
                logging.Trace(getIdCfg.sql);
                if (err) {
                    connection.end();
                    return f({ error: err.message });
                } else {
                    var idConfig = result && result.length > 0 && result[0].configuracion_id;
    ...
    connection.connect(function (err, usrs) {
        if (err) {
            ...
            var query = connection.query('SELECT p.nombre as name, p.idpasarela as idCGW, ' + 
                'DATE_FORMAT(p.ultima_actualizacion, "%d/%m/%Y %H:%i:%s") AS ultima_actualizacion, p.pendiente_actualizar, ' +
