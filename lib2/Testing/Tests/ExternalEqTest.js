...
exports.test = async ()=>{
    ...
    try {
        for (const item of testBench1){
            ...
            assert(Array.isArray(res) && res.length===item.out, "getExternalResources, Error in " + JSON.stringify(item));    
        }
    ...
    try {
        for (const item of testBench2){
            ...
            assert(Array.isArray(res) && res.length===item.out, "getRadioExternalResources, Error in " + JSON.stringify(item));    
        }
    ...
    try {
        for (const item of testBench3){
            ...
            assert(Array.isArray(res) && res.length===item.out, "filterExternalResources, Error in " + JSON.stringify(item));    
        }
    ...
    try {
        for (const item of testBench4){
            ...
            assert(Array.isArray(res) && res.length===item.out, "filterExternalPhoneResources, Error in " + JSON.stringify(item));    
        }
    ...
    try {
        for (const item of testBench5){
            ...
            assert(Array.isArray(res) && res.length===1 && res[0].alias===item.out1, "getExternalResource, Error in " + JSON.stringify(item));
            res = await lib.postExternalResource(item.in2);
            assert(res.affectedRows===1,"postExternalResource Error 1 in "+ JSON.stringify(item))
            item.in3.id_recurso = res.insertId;
            res = await lib.postExternalResource(item.in3);
            assert(res.affectedRows===1,"postExternalResource Error 2 in "+ JSON.stringify(item))
            res = await lib.deleteExternalResource(item.in3.id_recurso);
            assert(res.affectedRows===1,"deleteExternalResource Error in "+ JSON.stringify(item))
        }
